...
namespace MyNamespace
{
    public class SphereGenerator : MonoBehaviour
    {
        ...
        private SimpleMeshData[] _meshData;
        ...
        [Button("Clear", ButtonAlignment = 0.5f)]
        ...
        [Button("Generate Sphere", ButtonAlignment = 0.5f)]
        private void GenerateSphere()
        {
            _meshData = GenerateMeshes(resolution, subDivisions, radius);
            _renderObjects = new RenderObject[_meshData.Length];

            for (var index = 0; index < _meshData.Length; index++)
            {
                var mesh = _meshData[index];

                int matIndex = index % mats.Count;
                var mat = mats[matIndex];

                _renderObjects[index] = MeshHelper.CreateRendererObject("dank", mesh, mat, transform);
            }
        }
