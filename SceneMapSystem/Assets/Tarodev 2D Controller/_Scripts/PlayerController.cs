...
namespace TarodevController
{
    ...
    public class PlayerController : MonoBehaviour, IPlayerController
    {
        ...
        protected virtual void CheckCollisions()
        {
            ...
            // Ground and Ceiling
            _groundHitCount = Physics2D.CapsuleCastNonAlloc( _col.bounds.center, _col.size, _col.direction, 0, Vector2.down, _groundHits,
                _stats.GrounderDistance, ~_stats.PlayerLayer );
            _ceilingHitCount = Physics2D.CapsuleCastNonAlloc( _col.bounds.center, _col.size, _col.direction, 0, Vector2.up, _ceilingHits,
                _stats.GrounderDistance, ~_stats.PlayerLayer );
            ...
            _hittingWall = Physics2D.CapsuleCast( _col.bounds.center, _col.size, _col.direction, 0, new Vector2( _input.FrameInput.Move.x, 0 ),
                _stats.GrounderDistance, ~_stats.PlayerLayer );
        ...
        protected virtual bool TryGetLedgeCorner( out Vector2 cornerPos )
        {
            ...
            var hit1 = Physics2D.Raycast( grabHeight + _stats.LedgeRaycastSpacing * Vector2.down, WallDirection * Vector2.right, 0.5f,
                _stats.ClimbableLayer );
            if ( !hit1.collider ) return false; // Should hit below the ledge. Mainly used to determine xPos accurately

            var hit2 = Physics2D.Raycast( grabHeight + _stats.LedgeRaycastSpacing * Vector2.up, WallDirection * Vector2.right, 0.5f,
                _stats.ClimbableLayer );
            if ( hit2.collider )
                return false; // we only are within ledge-grab range when the first hits and second doesn't

            var hit3 = Physics2D.Raycast( grabHeight + new Vector2( WallDirection * 0.5f, _stats.LedgeRaycastSpacing ), Vector2.down, 0.5f,
                _stats.ClimbableLayer );
        ...
        {
            transform.position =
                _rb.position = _ledgeCornerPos + new Vector2( _stats.LedgeClimbTeleportDistance * _lastWallDirection, 0 ) +
                               Vector2.Scale( _stats.StandUpOffset, new( WallDirection, 1f ) );
        ...
        protected virtual bool ShouldMountLadder => _stats.AutoAttachToLadders || FrameInput.Move.y > _stats.VerticalDeadzoneThreshold ||
                                                    ( !_grounded && FrameInput.Move.y < -_stats.VerticalDeadzoneThreshold );

        protected virtual bool ShouldDismountLadder =>
            !_stats.AutoAttachToLadders && _grounded && FrameInput.Move.y < -_stats.VerticalDeadzoneThreshold;
        ...
        protected virtual bool CanWallJump => ( _isOnWall && !_isLeavingWall ) ||
        ...
        protected virtual void HandleHorizontal()
        {
            ...
            else
            {
                ...
                _speed.x = Mathf.MoveTowards( _speed.x, xInput * _stats.MaxSpeed,
        ...
        protected virtual void ApplyMovement()
        {
            ...
            _currentExternalVelocity =
                Vector2.MoveTowards( _currentExternalVelocity, Vector2.zero, _stats.ExternalVelocityDecay * Time.fixedDeltaTime );
        }
        ...
        private void OnValidate()
        {
            ...
            if ( _rb == null && !TryGetComponent( out _rb ) )
                Debug.LogWarning( "Ensure the GameObject with the Player Controller has a Rigidbody2D", this );
        }
#endif
    }
